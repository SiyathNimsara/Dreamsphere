...
import { SLEEP_STAGES } from '../../data/researchData';
import Card from '../common/Card';

export default function SleepCycleVisualizer() {
  const [selectedStageIdx, setSelectedStageIdx] = useState(4); // REM default
  const [hourTime, setHourTime] = useState(6);
  ...
  return (
    <Card hoverGlow={false} className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-950/60 p-5 rounded-2xl border border-white/10 space-y-3">
          <span className="text-xs text-pink-400 font-mono font-semibold block">Cognitive Function</span>
          <p className="text-xs text-slate-300 leading-relaxed">
            {selectedStageIdx === 4
              ? 'Emotional processing, creative synthesis, and synaptic rebalancing of daytime experiences.'
              : selectedStageIdx === 3
              ? 'Glymphatic brain clearance of metabolic waste, growth hormone surge, and deep tissue repair.'
